...
| Step 8a | Done (runtime tested 2026-05-08) | step-8a-foreground-service | Two sub-steps. (1) Data path: `DepartureDto` extended with `scheduled` / `expected` / `state` per Trafiklab OpenAPI; `StopRepository.getNextDeparture` (with anti-BUG-009 docstring noting SL `journey.id` ≠ GTFS-RT `trip_id`); `TrackingState.Polling` carries `nextDeparture` + `matchedDirection`; `LivePositionTracker` fetches departures every 60s alongside deviations. New `util.GeoMath` (haversine + project-onto-segment), new `widget.WidgetCommuteState` + `Phase` enum + `WidgetDeviationSummary`, new `widget.WidgetStateDeriver` (pure object — busIndex via consecutive-stops projection, vehicle-lock heuristic, 7-state Phase derivation). MainActivity logs the derived widget state per tick. (2) Foreground service: `service.CommuteTrackingService` runs its own `LivePositionTracker` instance off-Activity; persistent notification (IMPORTANCE_MIN, low priority); writes derived state into singleton `widget.WidgetStateHolder`; self-stops on `NoActiveCommute`. New permissions: FOREGROUND_SERVICE, FOREGROUND_SERVICE_DATA_SYNC, POST_NOTIFICATIONS. New vector drawable for notification icon. New string resources for channel + notification text. MainActivity starts the service in onResume when there's an active commute window AND from the commute_saved fragment-result handler (added after first runtime test caught zero-saved-commutes-at-launch path); also requests POST_NOTIFICATIONS at runtime via ActivityResultContracts.RequestPermission. Three commits on branch: `c95b48f` data path, `35e18e1` foreground service, `b38525e` runtime fixes. Runtime confirmed end-to-end on bus 57 → Sofia from Nätgränd: service started cleanly, both trackers polled in parallel, busIndex monotonically incremented from 25.03 → 27.00 over ~3 min as the bus moved, ETA ticked down naturally 8 → 7 → ... → 0 min, then at 17:21:01 the SL Departures feed correctly rotated to the next bus showing delta=+5min late (predicted-vs-scheduled, surfaced by SL not computed by us — captures a real delay event in live data). Phase stayed Deviation throughout because of an active line-57 deviation ("Hållplats Jungfrugatan flyttad") — correct per design handoff's state-priority order (Deviation > Late). |
| Step 8b | Done (sub-steps 2.0–2.9 runtime-confirmed; sub-steps 2.10–2.12 visual confirmation pending — user signalled closeout by running /cost on 2026-05-09) | step-8b-widget-surface | AppWidget surface end-to-end. New widget package: `CommuteAppWidgetProvider` (onUpdate / onDeleted, companion `updateAll(ctx)` for service-side push), `WidgetConfigActivity` (AlertDialog picker for CommuteConfig, empty-state handling, `setResult(RESULT_OK)` per APPWIDGET_CONFIGURE protocol), `WidgetBindingStore` (SharedPreferences widgetId → identity persistence), `WidgetCommuteIdentity` (`from(config)` returns `"$stopId\|$lineId\|$direction\|$timeWindowStart"`), `WidgetRenderer` (pure RemoteViews binder), `WidgetBitmapRenderer` (Canvas → Bitmap for route gauge + time-scale gauge — RemoteViews has no Canvas composable, so bitmap-rendered geometry is the only option). `widget_commute.xml` layout: header (line badge + → destination · from stop), route line ImageView (28dp Canvas bitmap), stop labels (leftmost-visible + ★ user stop), conditional `widget_stops_away` line ("N stops away" amber #FFB74D when bus out of 5-stop window), bottom row vertical-stack symmetric (left: hero caption "Estimated arrival" + bold ETA / right: scheduled-arrival caption "Scheduled arrival: HH:mm → HH:mm" + scale gauge + delta "+5 min late"), conditional Chronometer for GPS age ("Updated MM:SS ago", auto-ticks every second inside launcher's process), conditional deviation pill (amber #FFB300 with totalCount badge). All colours hardcoded for dark mode (BUG-020 deferred). `CommuteTrackingService` (Step 8a) calls `CommuteAppWidgetProvider.updateAll(ctx)` per tick — both sides render identically through the same `WidgetRenderer.render()`. Visual iteration loop produced 9 BUGS.md entries: BUG-014 (route windowed to 5 stops), BUG-015 (scheduled time slot), BUG-016 (GPS-age, three-version arc culminating in Chronometer), BUG-017 (off-gauge chip → dedicated stops-away line), BUG-018 (hero ETA caption), BUG-019 (dark-mode-only), BUG-020 (light-mode deferred), BUG-021 (stops-away wording, two-version arc), BUG-022 (bus marker clipping), BUG-023 (header times legibility, two-version arc). One discovered-during-Step-8b-but-deferred bug: BUG-024 (line picker hides off-service lines). Branch commits: pre-compact arc (sub-steps 2.0–2.9, ~10 commits), post-compact arc `e15985c` (2.10), `a2f909d` (2.11), `e509965` (2.12), `433620e` (BUG-024 file). Cost: $8.85 captured for closing arc; pre-compact arc unmeasured (context overflowed). |
| Step 9 | Done (covered by Step 8b — code already on main 2026-05-09) | step-9-lockscreen-to-app (no code change; documentation-only branch) | **Discovered already implemented** during Step 9 kickoff: the tap-to-open PendingIntent was added inline during Step 8b's widget-builder code at `CommuteAppWidgetProvider.kt:108-120` — `setOnClickPendingIntent(R.id.widget_root, ...)` launches MainActivity with `FLAG_ACTIVITY_NEW_TASK \| FLAG_ACTIVITY_SINGLE_TOP` on any tap of the widget root. Lock-screen eligibility is inherited automatically on Android 16 QPR1+ where lock-screen widgets are opt-out by default (see `widget_commute_info.xml` comment + PLAN.md change-log 2026-05-08). On older Android versions lock-screen widgets aren't supported at the OS level — `keyguard` widgetCategory was removed in Android 5.0 — and there's nothing the app can do about it. P1-FR11 functionally satisfied. **Runtime confirmation pending:** user should tap the widget once on next session to verify MainActivity opens cleanly (the rest of the path is mechanically obvious). No new branch commits needed; this row update + USAGE_LOG entry 109 is the entire Step 9 deliverable. |
| Step 10 | Done (runtime confirmation pending across BUG-029/030/031 + WINDOW=6 + leave-now/tap-zoom) | multiple branches merged into main 2026-05-09 | Step 10 polish run cleared the deferred-bug list (BUG-001 Swedish-character search, BUG-008 polyline overlap dedupe + per-config user-stop highlight, BUG-012 UNCERTAIN threshold 60s→120s, BUG-020 closed-without-fix per dark-only project decision, BUG-006 closed-without-fix per user judgement). Surfaced + fixed runtime regressions: BUG-029 (deviation card auto-expand wrong for long content — chevron + ScrollView), BUG-030 (widget Updated timer aligned to poll time, was on GPS time which lagged 30-90s). Resolved P1-FR8 spec-gap as BUG-031 — full ServiceAlerts.pb integration (new `model/TripAlert.kt`, new `repository/ServiceAlertsRepository.kt`, `LivePositionTracker` extension, two-surface UI with widget-vs-app boundary rule: widget = trip-matched only, app = full picture). Three mid-run reverts (light-mode widget per dark-only memory, WINDOW 7→5 per runtime crowding, BUG-027 auto-expand per BUG-029). Final WINDOW tuning at 6 stops so a 5-stops-away bus is visible at the leftmost gauge dot. Drop "leave now!" hero text. Widget-tap → zoom to tracked vehicle (zoom 17 street-level) instead of route auto-fit. Cost: $44.04 over 2h 8m wall (Checkpoint 9), substantially elevated by the BUG-031 feature scope + high iteration density at >150k context. |
